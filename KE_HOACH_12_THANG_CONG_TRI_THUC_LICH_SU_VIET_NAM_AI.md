...
> Phạm vi đánh giá: mã nguồn, 165 Flow card, test, API/OpenAPI, giao diện, release evidence và DoD artifacts trong workspace hiện tại
...
|---|---|---|
| Flow planning | PASS; contract đã khóa; **165 card đã tạo, 165 done, C-165 hiện tại** | `flow/00-idea.md`…`flow/05-contract.md`, `bash /Users/admin/.agents/skills/flow/runner/flow.sh status` |
...
| Release quality | PASS_LOCAL_ONLY; source tree khớp với tested main revision `83f1a1b`; không phải official production | `artifacts/release/current-head-evidence.json` |
...
| Immutable container image | PASS_SUPPLY_CHAIN_ONLY; GHCR image publish job chạy trên commit `83f1a1b`, digest `sha256:5fdc06acddaad4b37577eea1ae78d76d786dd5a9c78d3552f0c762861ac3128e` đã lưu; chưa deploy production, Public Beta false | `cards/C-163.md`, `.github/workflows/container-publish.yml`, `docs/release-runbook.md` |
| GHCR runtime smoke | PASS_LOCAL_CONTAINER_ONLY; image theo digest chạy được, health/OpenAPI/search `200`, Docker health `healthy`; không phải official production | `cards/C-165.md`, `artifacts/operations/ghcr-runtime-smoke.json`, `artifacts/operations/ghcr-runtime-smoke.md` |
...
|---|---|---|
| `KE_HOACH_12_THANG_CONG_TRI_THUC_LICH_SU_VIET_NAM_AI.md` | **Global Master v4.5** | Nguồn duy nhất cho scope, roadmap, gate, KPI, DoD, blocker và mọi card mới |
...
|---|---:|---|
| Tiến độ Flow | **165/165 card sau khi C-165 được kiểm tra (100%)** | Nền tảng kỹ thuật, metadata/rights gate, AI/privacy/security handoff, packet lịch sử, Global Master, production preflight, merged-main release evidence, immutable image handoff và digest runtime smoke đã hoàn tất |
| Vị trí roadmap | **Cuối M11 / chuẩn bị M12** | Đang ở release-candidate hardening, chưa mở Public Beta |
| Release quality | **4/4 PASS_LOCAL_ONLY** | Tested main revision `83f1a1b`; local standalone, không phải domain production |
...
Flow hiện có **165 thẻ**: C-001 đến C-165; **165 thẻ done** và C-165 là card hiện tại cho GHCR runtime smoke. Các card C-028–C-038 đã được thực thi và bằng chứng của chúng nằm trong `artifacts/`/lịch sử git; không dùng số liệu 38-card cũ để báo cáo hiện trạng.

Kiểm tra ngày 11/08/2026 cũng phát hiện hai điều phải xử lý trước khi mở rộng:

1. Bằng chứng release gần nhất trong `artifacts/release` thuộc commit cũ hơn HEAD hiện tại; do đó cần release lại nhánh mới nhất trước khi gọi là “đã triển khai”.
