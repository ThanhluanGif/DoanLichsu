...
| 3 | Mô hình `content_nodes` + `content_translations` + subtype details + sources/media/relations; locale là `vi|en`, ngày có precision | Giữ một thực thể lịch sử ổn định khi slug/bản dịch thay đổi và cho locale switch theo id | Một bảng bài viết JSON khổng lồ: CRUD nhanh nhưng khó constraint nguồn, quan hệ, subtype, alternate locale và audit; một bảng riêng hoàn toàn cho từng type gây lặp translation/source logic |
