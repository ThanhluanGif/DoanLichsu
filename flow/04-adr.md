...
| 11 | Thêm `curriculum_requirements` và bảng nối content, tách khỏi `content_nodes`; requirement lưu grade, mandatory/elective, official-program ref, outcomes và sort | Một sự kiện có thể phục vụ nhiều lớp/chủ đề; coverage phải đo theo chương trình chứ không biến grade thành content type hoặc tag tùy ý | Hard-code mục lục trong React hoặc gắn một `grade` lên content: không biểu diễn được lớp 10–12 theo chủ đề, chuyên đề lựa chọn và quan hệ nhiều-nhiều |
| 12 | Facet được aggregate phía server từ đúng tập `PUBLISHED` theo scope, trả label/value/count; client không có danh sách option hard-code | Chặn option rỗng ngay tại seam, dùng chung cho collection/search/curriculum và giữ URL state ổn định | Fetch toàn bộ records để đếm ở browser hoặc giữ `includeEmpty=true`: không scale, dễ lệch count và tái tạo bug chọn vào 0 kết quả |
| 13 | Coverage có `publishedCount` và `verifiedCount` riêng; chỉ Reviewer workflow làm tăng verified, seed/migration không tự phong nguồn/claim là verified | Công khai tiến độ thật và giữ luật nguồn hiện có; `PUBLISHED` cũ không đồng nghĩa đạt chuẩn bài học | Boolean `complete` nhập tay hoặc suy từ số record: dễ grade-laundering và che requirement thiếu |
| 14 | Bản đồ dùng GeoJSON cục bộ + semantic HTML/SVG fallback; prototype 3D dùng scene data định trước và Three.js tải động, không phụ thuộc tile provider để đọc nội dung | Re-architect GIS/game C thành một reconstruction B có thể kiểm thử, cache và gắn source/assumption/confidence cho từng phase | Game engine/realtime physics, photogrammetry hoặc map tile SaaS bắt buộc: bundle/vận hành/quyền dữ liệu lớn và không làm tăng độ đúng sử liệu |
| 15 | Lesson tách `originalSummary`, `analysis`, `debates`, `asOf`; source asset thêm institution/inventory/origin/rights/permission/credit/checksum | Không trộn sự kiện, diễn giải và bản scan; URL công khai không được hiểu là giấy phép tái sử dụng | Copy nguyên sách/bài bảo tàng hoặc lưu ảnh chỉ với URL: rủi ro bản quyền, provenance và không thể audit |
| 16 | Motion theo progressive enhancement: brand loader dùng SVG hiện có, scene/route animation không khóa scroll hoặc LCP và bị vô hiệu hoàn toàn bởi reduced-motion | Tăng nhận diện/cảm giác khám phá mà vẫn giữ thứ tự đọc, keyboard và ngân sách hiệu năng | Loading overlay có thời lượng tối thiểu, parallax listener toàn trang hoặc animation-only navigation: trì hoãn nội dung và gây accessibility regression |
...
- Không AI/RAG/auto-translation, VR/AR, tour 3D toàn kho, GIS phụ thuộc dịch vụ ngoài,
  game/realtime physics hoặc lời hứa “mô phỏng chính xác tuyệt đối”; chỉ một map cục bộ và
  một scene giáo dục định trước được nhận vào V2.
...
- Không sao chép nguyên sách giáo khoa hoặc số hóa tư liệu lưu trữ khi chưa có quyền; URL
  nguồn chỉ tạo citation, không tự động tạo quyền phục vụ asset.
