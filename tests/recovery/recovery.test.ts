...
describe("backup and restore rehearsal",()=>{
  it("creates a verified snapshot and restores matching v1 counts",()=>{
    ...
    expect(backup.counts).toMatchObject({contentNodes:50,translations:100,sources:50,users:3});expect(backup.sha256).toMatch(/^[a-f0-9]{64}$/);
    ...
    expect(restore).toMatchObject({database:restoredPath,sha256Verified:true,schemaVersion:3,counts:backup.counts});
