...
async function trackLcp(page:Page){
  await page.addInitScript(()=>{
    const state=globalThis as typeof globalThis&{__grade6Lcp?:number};state.__grade6Lcp=0;
    new PerformanceObserver((list)=>{for(const entry of list.getEntries())state.__grade6Lcp=Math.max(state.__grade6Lcp??0,entry.startTime);}).observe({type:"largest-contentful-paint",buffered:true});
  });
}
...
async function metrics(page:Page){
  ...
  return page.evaluate(()=>{
    const state=globalThis as typeof globalThis&{__grade6Lcp?:number};
    return{clientWidth:document.documentElement.clientWidth,scrollWidth:document.documentElement.scrollWidth,lcpMs:state.__grade6Lcp??0};
  });
}
...
test("Grade 6 reaches 8/8 verified coverage and a grounded lesson by keyboard on desktop and mobile",async({browser,baseURL})=>{
  test.setTimeout(120_000);if(!baseURL)throw new Error("baseURL is required");
  await mkdir("artifacts/curriculum/grade-6",{recursive:true});

  const requestContext=await browser.newContext();
  const viResponse=await requestContext.request.get(`${baseURL}/api/v1/vi/curriculum/6?track=MANDATORY&pageSize=50`);
  const enResponse=await requestContext.request.get(`${baseURL}/api/v1/en/curriculum/6?track=MANDATORY&pageSize=50`);
  ...
  for(const grade of [viEnvelope.data,enEnvelope.data]){
    ...
    expect(grade.requirements).toHaveLength(8);
    expect(grade.requirements.every((item)=>item.coverageStatus==="VERIFIED"&&item.publishedCount>0&&item.verifiedCount>0)).toBe(true);
  }
  await writeFile("artifacts/curriculum/grade-6/grade-6-api-vi.json",`${JSON.stringify(viEnvelope,null,2)}\n`);
  await writeFile("artifacts/curriculum/grade-6/grade-6-api-en.json",`${JSON.stringify(enEnvelope,null,2)}\n`);
  ...
  await desktop.goto(`${baseURL}/vi/hoc-theo-lop/6`,{waitUntil:"networkidle"});
  await expect(desktop.getByRole("heading",{level:1,name:"Lớp 6",exact:true})).toBeVisible();
  await expect(desktop.locator('.curriculum-requirement[data-coverage-status="VERIFIED"]')).toHaveCount(8);
  expect(await desktop.locator(".curriculum-coverage-strip strong").allTextContents()).toEqual(["8","8","8"]);
  ...
  await desktop.screenshot({path:"artifacts/curriculum/grade-6/grade-6-coverage-desktop.png",fullPage:true});
  const viLesson=viEnvelope.data.requirements.find(({id})=>id==="g6-why-study-history")!.lessons.find(({id})=>id==="lesson-g6-why-study-history")!;
  ...
  expect(viDetail).toMatchObject({id:viLesson.id,asOf:"2026-08-11T00:00:00.000Z",reviewedBy:expect.stringContaining("C-028")});
  expect(viDetail.claims).toHaveLength(2);expect(viDetail.claims.every((claim)=>claim.evidence.length===1&&claim.evidence[0].source.verificationStatus==="VERIFIED")).toBe(true);
  ...
  await desktop.screenshot({path:"artifacts/curriculum/grade-6/grade-6-desktop.png",fullPage:true});await desktopContext.close();
  ...
  await mobile.goto(`${baseURL}/en/learn-by-grade/6`,{waitUntil:"networkidle"});
  await expect(mobile.getByRole("heading",{level:1,name:"Grade 6",exact:true})).toBeVisible();
  await expect(mobile.locator('.curriculum-requirement[data-coverage-status="VERIFIED"]')).toHaveCount(8);
  ...
  await mobile.screenshot({path:"artifacts/curriculum/grade-6/grade-6-coverage-mobile.png",fullPage:true});
  const enLesson=enEnvelope.data.requirements.find(({id})=>id==="g6-funan")!.lessons.find(({id})=>id==="lesson-g6-funan")!;
  ...
  await mobile.screenshot({path:"artifacts/curriculum/grade-6/grade-6-mobile.png",fullPage:true});await mobileContext.close();

  const proof={
    generatedAt:new Date().toISOString(),url:new URL(baseURL).origin,
    coverage:{requirements:8,published:8,verified:8,fullCoverage:true,locales:["vi","en"]},
    ...
    screenshots:[
      "artifacts/curriculum/grade-6/grade-6-coverage-desktop.png","artifacts/curriculum/grade-6/grade-6-desktop.png",
      "artifacts/curriculum/grade-6/grade-6-coverage-mobile.png","artifacts/curriculum/grade-6/grade-6-mobile.png",
    ],
  };
  await writeFile("artifacts/curriculum/grade-6/grade-6-proof.json",`${JSON.stringify(proof,null,2)}\n`);
});
