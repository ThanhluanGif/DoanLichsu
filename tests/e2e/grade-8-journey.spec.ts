...
async function trackLcp(page:Page){
  await page.addInitScript(()=>{
    const state=globalThis as typeof globalThis&{__grade8Lcp?:number};state.__grade8Lcp=0;
    new PerformanceObserver((list)=>{for(const entry of list.getEntries())state.__grade8Lcp=Math.max(state.__grade8Lcp??0,entry.startTime);}).observe({type:"largest-contentful-paint",buffered:true});
  });
}
...
async function metrics(page:Page){
  ...
  return page.evaluate(()=>{
    const state=globalThis as typeof globalThis&{__grade8Lcp?:number};
    return{clientWidth:document.documentElement.clientWidth,scrollWidth:document.documentElement.scrollWidth,lcpMs:state.__grade8Lcp??0};
  });
}
...
test("Grade 8 reaches 7/7 verified coverage and grounded lessons by keyboard on desktop and mobile",async({browser,baseURL})=>{
  test.setTimeout(120_000);if(!baseURL)throw new Error("baseURL is required");
  await mkdir("artifacts/curriculum/grade-8",{recursive:true});

  const requestContext=await browser.newContext();
  const viResponse=await requestContext.request.get(`${baseURL}/api/v1/vi/curriculum/8?track=MANDATORY&pageSize=50`);
  const enResponse=await requestContext.request.get(`${baseURL}/api/v1/en/curriculum/8?track=MANDATORY&pageSize=50`);
  ...
  for(const grade of [viEnvelope.data,enEnvelope.data]){
    ...
    expect(grade.requirements).toHaveLength(7);
    expect(grade.requirements.every((item)=>item.coverageStatus==="VERIFIED"&&item.publishedCount>0&&item.verifiedCount>0)).toBe(true);
  }
  await writeFile("artifacts/curriculum/grade-8/grade-8-api-vi.json",`${JSON.stringify(viEnvelope,null,2)}\n`);
  await writeFile("artifacts/curriculum/grade-8/grade-8-api-en.json",`${JSON.stringify(enEnvelope,null,2)}\n`);
  ...
  await desktop.goto(`${baseURL}/vi/hoc-theo-lop/8`,{waitUntil:"networkidle"});
  await expect(desktop.getByRole("heading",{level:1,name:"Lớp 8",exact:true})).toBeVisible();
  await expect(desktop.locator('.curriculum-requirement[data-coverage-status="VERIFIED"]')).toHaveCount(7);
  expect(await desktop.locator(".curriculum-coverage-strip strong").allTextContents()).toEqual(["7","7","7"]);
  ...
  await desktop.screenshot({path:"artifacts/curriculum/grade-8/grade-8-coverage-desktop.png",fullPage:true});
  const viLesson=viEnvelope.data.requirements.find(({id})=>id==="g8-early-modern-revolutions")!.lessons.find(({id})=>id==="lesson-g8-early-modern-revolutions")!;
  ...
  expect(viDetail).toMatchObject({id:viLesson.id,asOf:"2026-08-11T00:00:00.000Z",reviewedBy:expect.stringContaining("C-030")});
  ...
  await desktop.screenshot({path:"artifacts/curriculum/grade-8/grade-8-desktop.png",fullPage:true});await desktopContext.close();
  ...
  await mobile.goto(`${baseURL}/en/learn-by-grade/8`,{waitUntil:"networkidle"});
  await expect(mobile.getByRole("heading",{level:1,name:"Grade 8",exact:true})).toBeVisible();
  await expect(mobile.locator('.curriculum-requirement[data-coverage-status="VERIFIED"]')).toHaveCount(7);
  ...
  await mobile.screenshot({path:"artifacts/curriculum/grade-8/grade-8-coverage-mobile.png",fullPage:true});
  const enLesson=enEnvelope.data.requirements.find(({id})=>id==="g8-vietnam-1885-1918")!.lessons.find(({id})=>id==="lesson-g8-vietnam-1885-1918")!;
  ...
  const enDetail=(await enDetailResponse.json()).data as DetailView;expect(enDetail).toMatchObject({asOf:"2026-08-11T00:00:00.000Z",reviewedBy:expect.stringContaining("C-030")});expect(enDetail.claims).toHaveLength(2);expect(enDetail.sources.every((source)=>source.verificationStatus==="VERIFIED")).toBe(true);
  ...
  await mobile.screenshot({path:"artifacts/curriculum/grade-8/grade-8-mobile.png",fullPage:true});await mobileContext.close();

  const proof={
    generatedAt:new Date().toISOString(),url:new URL(baseURL).origin,
    coverage:{requirements:7,published:7,verified:7,fullCoverage:true,locales:["vi","en"]},
    ...
    screenshots:[
      "artifacts/curriculum/grade-8/grade-8-coverage-desktop.png","artifacts/curriculum/grade-8/grade-8-desktop.png",
      "artifacts/curriculum/grade-8/grade-8-coverage-mobile.png","artifacts/curriculum/grade-8/grade-8-mobile.png",
    ],
  };
  await writeFile("artifacts/curriculum/grade-8/grade-8-proof.json",`${JSON.stringify(proof,null,2)}\n`);
});
