...
import { sourceTreeSha256 } from "../scripts/source-tree-hash.mjs";
...
const checks = [
  ...
  { id: "current-head-release-evidence", group: "quality", status: currentRelease.steps?.filter((step) => ["lint", "typecheck", "test", "build"].includes(step.name)).every((step) => step.exitCode === 0) && currentRelease.local?.health === 200 && currentReleaseCoherent ? "PASS_LOCAL_ONLY" : "BLOCKED", evidence: ["artifacts/release/current-head-evidence.json"], sourceTreeSha256: currentRelease.sourceTreeSha256 ?? null, currentSourceTreeSha256, sourceTreeMatches: currentReleaseCoherent },
