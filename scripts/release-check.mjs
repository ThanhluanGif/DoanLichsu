...
const commit=execFileSync("git",["rev-parse","HEAD"],{cwd:root,encoding:"utf8"}).trim();const startedSourceTreeSha256=sourceTreeSha256();const steps=[];const captured=[];
...
const report={startedAt,finishedAt:new Date().toISOString(),version,testedCommit:commit,sourceTreeSha256:startedSourceTreeSha256,passed:failure===null,steps,recoveryProof,dependencyAuditSummary:dependencyAudit?.metadata?.vulnerabilities??null,liveProof,scope:{localRecoveryAndContract:"disposable local production-like evidence; not official production",httpsE2e:"requires E2E_BASE_URL set to a deployed HTTPS origin",officialProduction:false},logScan:{configuredSecrets:secrets.length,exposed:false,patterns:["set-cookie:","qsv_session=","password_hash","session_secret","authorization:"]},failure:failure?redact(failure):null};
await writeFile(resolve(artifactDirectory,"release-report.json"),`${JSON.stringify(report,null,2)}\n`);
const markdown=["# Release check",`- Result: **${report.passed?"PASS":"FAIL"}**`,`- Version: \`${version}\``,`- Tested commit: \`${commit}\``,`- Source tree SHA-256: \`${startedSourceTreeSha256}\``,`- Started: ${startedAt}`,`- Finished: ${report.finishedAt}`,"","## Scope disclosure","",`- Local recovery/contract evidence: **disposable production-like only; not official production**`,`- HTTPS E2E: **requires E2E_BASE_URL deployed HTTPS origin**`,"","## Steps","",...steps.map((step)=>`- ${step.exitCode===0?"PASS":"FAIL"} \`${step.command}\` — ${step.durationMs} ms`),"",`- Recovery: ${recoveryProof?`SHA-256 verified; ${recoveryProof.restore.counts.contentNodes} nodes / ${recoveryProof.restore.counts.translations} translations / ${recoveryProof.restore.counts.sources} sources / ${recoveryProof.restore.counts.users} users; restored HTTP health ${recoveryProof.restoredHttp.health}`:"not completed"}`,`- Dependency High/Critical: ${report.dependencyAuditSummary?`${report.dependencyAuditSummary.high}/${report.dependencyAuditSummary.critical}`:"not completed"}`,`- Sensitive log scan: ${report.logScan.exposed?"FAIL":"PASS"}`,failure?`- Failure: ${report.failure}`:"",""].filter((line)=>line!==null);
