...
function validationViolations(database:SqliteDatabase,id:string,selected:Locale[]):string[]{const violations:string[]=[];const node=database.prepare("SELECT type,start_date,end_date,date_precision FROM content_nodes WHERE id=?").get(id) as{type:string;start_date:string|null;end_date:string|null;date_precision:string|null};if(node.type==="PERIOD"&&(!node.start_date||!node.end_date||!node.date_precision))violations.push("period requires startDate, endDate and datePrecision");const sourceCount=(database.prepare("SELECT COUNT(*) AS count FROM content_sources WHERE content_id=?").get(id) as{count:number}).count;if(sourceCount===0)violations.push("content requires at least one source");for(const locale of selected){const t=database.prepare("SELECT title,slug,summary,body,seo_title,seo_description,translation_status FROM content_translations WHERE node_id=? AND locale=?").get(id,locale) as Record<string,string>|undefined;if(!t)violations.push(`${locale}: translation is missing`);else{for(const field of ["title","slug","summary","body","seo_title","seo_description"]){if(!t[field]?.trim())violations.push(`${locale}: ${field} is required`);}if(t.translation_status!=="APPROVED")violations.push(`${locale}: translation must be APPROVED`);}}
