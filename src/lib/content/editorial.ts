...
function mediaInput(input:Record<string,unknown>,includeVersion:boolean){const kind=enumValue(input,"kind",["IMAGE","DOCUMENT"] as const,true)!;const credit=stringField(input,"credit",{required:true,max:500})!;const license=stringField(input,"license",{required:true,max:500})!;const altVi=stringField(input,"altVi",{required:true,max:1000})!;const altEn=stringField(input,"altEn",{required:true,max:1000})!;const rightsStatus=enumValue(input,"rightsStatus",rightsStatuses)??"LINK_ONLY";const permissionDocument=optionalHttpsUrl(input,"permissionDocument");if(rightsStatus==="PERMITTED"&&!permissionDocument)invalidField("permissionDocument","Media PERMITTED cần link giấy phép HTTPS.");const checksum=stringField(input,"checksum",{max:64});if(checksum&&!/^[a-f0-9]{64}$/.test(checksum))invalidField("checksum","Checksum phải là SHA-256 hex 64 ký tự.");for(const [field,value] of Object.entries({credit,license,altVi,altEn}))rejectMarkup(value,field);return{version:includeVersion?numberField(input,"version",true)!:undefined,url:httpsUrl(input,"url"),kind,credit,license,altVi,altEn,captionVi:stringField(input,"captionVi",{max:2000}),captionEn:stringField(input,"captionEn",{max:2000}),holdingInstitution:stringField(input,"holdingInstitution",{max:500})??"Chưa xác định",inventoryId:stringField(input,"inventoryId",{max:200}),origin:stringField(input,"origin",{max:2_000})??"Tài liệu tham chiếu bên ngoài; dự án không lưu bản sao.",rightsStatus,permissionDocument,creditLine:stringField(input,"creditLine",{max:500})??credit,checksum};}
function mediaView(row:Record<string,unknown>){return{id:row.id,url:row.url,kind:row.kind,credit:row.credit,license:row.license,alt:row.alt_vi,caption:row.caption_vi,width:row.width,height:row.height,provenance:{holdingInstitution:row.holding_institution,inventoryId:row.inventory_id,origin:row.origin,rightsStatus:row.rights_status,permissionDocument:row.permission_document,creditLine:row.credit_line,checksum:row.checksum},version:row.version,altVi:row.alt_vi,altEn:row.alt_en,captionVi:row.caption_vi,captionEn:row.caption_en};}
...
export function createMedia(database:SqliteDatabase,input:Record<string,unknown>,actor:AuthUser){const value=mediaInput(input,false);const id=randomUUID();const now=new Date().toISOString();database.transaction(()=>{database.prepare(`INSERT INTO media(id,url,kind,credit,license,alt_vi,alt_en,caption_vi,caption_en,width,height,holding_institution,inventory_id,origin,rights_status,permission_document,credit_line,checksum,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?,?,NULL,NULL,?,?,?,?,?,?,?,?,?)`).run(id,value.url,value.kind,value.credit,value.license,value.altVi,value.altEn,value.captionVi??null,value.captionEn??null,value.holdingInstitution,value.inventoryId??null,value.origin,value.rightsStatus,value.permissionDocument??null,value.creditLine,value.checksum??null,now,now);writeAudit(database,{actorId:actor.id,action:"media.create",objectType:"media",objectId:id});}).immediate();return mediaView(database.prepare("SELECT * FROM media WHERE id=?").get(id) as Record<string,unknown>);}
export function updateMedia(database:SqliteDatabase,id:string,input:Record<string,unknown>,actor:AuthUser){const value=mediaInput(input,true);const now=new Date().toISOString();database.transaction(()=>{if(database.prepare("SELECT 1 FROM content_media cm JOIN content_nodes n ON n.id=cm.content_id WHERE cm.media_id=? AND n.status='PUBLISHED' LIMIT 1").get(id))throw new ApiError(422,"PUBLISHED_MEDIA_IMMUTABLE","Media đã gắn với nội dung xuất bản; hãy tạo media mới và đi lại workflow.");const result=database.prepare(`UPDATE media SET url=?,kind=?,credit=?,license=?,alt_vi=?,alt_en=?,caption_vi=?,caption_en=?,holding_institution=?,inventory_id=?,origin=?,rights_status=?,permission_document=?,credit_line=?,checksum=?,version=version+1,updated_at=? WHERE id=? AND version=?`).run(value.url,value.kind,value.credit,value.license,value.altVi,value.altEn,value.captionVi??null,value.captionEn??null,value.holdingInstitution,value.inventoryId??null,value.origin,value.rightsStatus,value.permissionDocument??null,value.creditLine,value.checksum??null,now,id,value.version);if(result.changes!==1){if(!database.prepare("SELECT 1 FROM media WHERE id=?").get(id))throw new ApiError(404,"MEDIA_NOT_FOUND","Media không tồn tại.");throw new ApiError(409,"STALE_VERSION","Phiên bản media đã thay đổi.");}writeAudit(database,{actorId:actor.id,action:"media.update",objectType:"media",objectId:id});}).immediate();return mediaView(database.prepare("SELECT * FROM media WHERE id=?").get(id) as Record<string,unknown>);}
