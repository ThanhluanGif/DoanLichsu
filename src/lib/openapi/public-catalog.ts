...
export const publicOpenApiPaths = {
  "/api/v1/{locale}/ai/answer": { post: {
    operationId: "answerInternalAlphaTutor", summary: "Trả lời trợ giảng grounded internal alpha", description: "Chỉ hoạt động với header x-qsv-ai-alpha: internal và cookie phiên được cấp; Public Beta mặc định bị khóa. Câu trả lời phải có citation hoặc abstain.", tags: ["AI"], security: [{ cookieAuth: [] }], parameters: [localeParameter, { name: "x-qsv-ai-alpha", in: "header", required: false, schema: { type: "string", enum: ["internal"] } }], requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["question"], properties: { question: { type: "string", minLength: 1, maxLength: 2000 }, contextSlug: { type: "string", maxLength: 200 }, alpha: { type: "boolean" } } } } } }, responses: { "200": jsonResponse({ $ref: "#/components/schemas/AiAnswerResponse" }), "401": jsonResponse({ $ref: "#/components/schemas/ApiError" }, "Alpha requires an authenticated invited user."), "403": jsonResponse({ $ref: "#/components/schemas/ApiError" }, "Public AI beta is disabled."), ...errors("400", "500") },
  } },
  "/api/v1/{locale}/reconstructions": { get: {
    operationId: "listPublicReconstructions", summary: "Liệt kê tái dựng giáo dục", description: "Trả các scene đã xuất bản với nhãn interpretation, confidence và fallback narrative.", tags: ["Public"], parameters: [localeParameter, ...pageParameters], responses: { "200": jsonResponse(listRef("ReconstructionListItem")), ...errors("400", "404", "500") },
  } },
  "/api/v1/{locale}/reconstructions/{slug}": { get: {
    operationId: "getPublicReconstruction", summary: "Đọc một tái dựng giáo dục", description: "Trả phase, move, place, source, assumption và fallback; không phải tư liệu sự thật tuyệt đối.", tags: ["Public"], parameters: [localeParameter, { name: "slug", in: "path", required: true, schema: { type: "string" } }], responses: { "200": jsonResponse(dataRef("ReconstructionView")), ...errors("404", "500") },
  } },
...
export const publicOpenApiSchemas = {
  ReconstructionListItem: { type: "object", additionalProperties: false, required: ["id", "slug", "title", "summary", "label", "confidence", "thumbnail"], properties: { id: { type: "string" }, slug: { type: "string" }, title: { type: "string" }, summary: { type: "string" }, label: { type: "string", const: "EDUCATIONAL_RECONSTRUCTION" }, confidence: { type: "string", enum: ["HIGH", "MEDIUM", "LOW"] }, thumbnail: { anyOf: [{ $ref: "#/components/schemas/MediaView" }, { type: "null" }] } } },
  ReconstructionMove: { type: "object", additionalProperties: false, required: ["id", "side", "label", "from", "to", "confidence", "sourceIds"], properties: { id: { type: "string" }, side: { type: "string" }, label: { type: "string" }, from: { $ref: "#/components/schemas/GeoPoint" }, to: { $ref: "#/components/schemas/GeoPoint" }, confidence: { type: "string", enum: ["HIGH", "MEDIUM", "LOW"] }, sourceIds: { type: "array", items: { type: "string" } } } },
  GeoPoint: { type: "object", additionalProperties: false, required: ["longitude", "latitude"], properties: { longitude: { type: "number" }, latitude: { type: "number" } } },
  ReconstructionPhase: { type: "object", additionalProperties: false, required: ["id", "order", "title", "narrative", "dateLabel", "confidence", "assumptions", "focusPlaceIds", "moves"], properties: { id: { type: "string" }, order: { type: "integer" }, title: { type: "string" }, narrative: { type: "string" }, dateLabel: { type: "string" }, confidence: { type: "string", enum: ["HIGH", "MEDIUM", "LOW"] }, assumptions: { type: "array", items: { type: "string" } }, focusPlaceIds: { type: "array", items: { type: "string" } }, moves: { type: "array", items: { $ref: "#/components/schemas/ReconstructionMove" } } } },
  ReconstructionView: { type: "object", additionalProperties: false, required: ["id", "slug", "title", "summary", "label", "confidence", "thumbnail", "content", "assumptions", "sources", "places", "phases", "fallback"], properties: { id: { type: "string" }, slug: { type: "string" }, title: { type: "string" }, summary: { type: "string" }, label: { type: "string", const: "EDUCATIONAL_RECONSTRUCTION" }, confidence: { type: "string", enum: ["HIGH", "MEDIUM", "LOW"] }, thumbnail: { anyOf: [{ $ref: "#/components/schemas/MediaView" }, { type: "null" }] }, content: { $ref: "#/components/schemas/ContentListItem" }, assumptions: { type: "array", items: { type: "string" } }, sources: { type: "array", items: { $ref: "#/components/schemas/SourceView" } }, places: { type: "array", items: { $ref: "#/components/schemas/PlaceView" } }, phases: { type: "array", items: { $ref: "#/components/schemas/ReconstructionPhase" } }, fallback: { type: "object", additionalProperties: false, required: ["image", "narrative"], properties: { image: { anyOf: [{ type: "string" }, { type: "null" }] }, narrative: { type: "string" } } } } },
  ...
  AiCitation:{type:"object",additionalProperties:false,required:["sourceId","title","institution","locator","url"],properties:{sourceId:{type:"string"},title:{type:"string"},institution:nullableString,locator:{type:"string"},url:{type:"string",format:"uri"}}},
  AiAnswer:{type:"object",additionalProperties:false,required:["contractVersion","status","answer","keyPoints","citations","confidence","limitations","suggestedNext","generatedAt","modelVersion","promptPolicyVersion","corpusSnapshotId"],properties:{contractVersion:{type:"string",const:"ai-answer-v1"},status:{type:"string",enum:["GROUNDED","ABSTAIN"]},answer:{type:"string"},keyPoints:{type:"array",items:{type:"object",additionalProperties:false,required:["text","claimId"],properties:{text:{type:"string"},claimId:nullableString}}},citations:{type:"array",items:{$ref:"#/components/schemas/AiCitation"}},confidence:{type:"string",enum:["HIGH","MEDIUM","LOW"]},limitations:{type:"array",items:{type:"string"}},suggestedNext:{type:"array",items:{type:"object",additionalProperties:false,required:["type"],properties:{type:{type:"string",enum:["LESSON","SEARCH"]},slug:nullableString,title:nullableString,query:nullableString}}},generatedAt:{type:"string",format:"date-time"},modelVersion:{type:"string"},promptPolicyVersion:{type:"string"},corpusSnapshotId:{type:"string"},abstentionReason:nullableString,locale:{type:"string",enum:["vi","en"]}}},
  AiAnswerResponse:{type:"object",additionalProperties:false,required:["data","access"],properties:{data:{$ref:"#/components/schemas/AiAnswer"},access:{type:"string",const:"INTERNAL_ALPHA"}}},
