...
export const publicOpenApiPaths = {
  "/api/v1/{locale}/ai/answer": { post: {
    operationId: "answerInternalAlphaTutor", summary: "Trả lời trợ giảng grounded internal alpha", description: "Chỉ hoạt động với header x-qsv-ai-alpha: internal; Public Beta mặc định bị khóa. Câu trả lời phải có citation hoặc abstain.", tags: ["AI"], parameters: [localeParameter, { name: "x-qsv-ai-alpha", in: "header", required: false, schema: { type: "string", enum: ["internal"] } }], requestBody: { required: true, content: { "application/json": { schema: { type: "object", required: ["question"], properties: { question: { type: "string", minLength: 1, maxLength: 2000 }, alpha: { type: "boolean" } } } } } }, responses: { "200": jsonResponse({ $ref: "#/components/schemas/AiAnswerResponse" }), "403": jsonResponse({ $ref: "#/components/schemas/ApiError" }, "Public AI beta is disabled."), ...errors("400", "500") },
  } },
