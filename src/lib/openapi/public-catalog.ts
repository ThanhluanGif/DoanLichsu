...
export const publicOpenApiSchemas = {
  ...
  PublicSourceItem: { type: "object", additionalProperties: false, required: ["id", "title", "author", "publisher", "year", "url", "accessedAt", "citationNote", "contentCount", "contents"], properties: { id: { type: "string" }, title: { type: "string" }, author: nullableString, publisher: nullableString, year: { anyOf: [{ type: "integer" }, { type: "null" }] }, url: { type: "string", format: "uri" }, accessedAt: { type: "string", format: "date-time" }, citationNote: nullableString, contentCount: { type: "integer", minimum: 1 }, contents: { type: "array", items: { $ref: "#/components/schemas/SourceContentRef" } } } },
  PeriodRef: periodRef,
