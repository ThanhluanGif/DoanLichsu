...
export function PublicShell({locale,localeHref,children}:{locale:Locale;localeHref:string|null;children:React.ReactNode}) {
  ...
  return <div className="public-site" lang={locale}>
    ...
    <footer className="site-footer">
      ...
      <div className="footer-editorial"><p>{locale === "vi" ? "Dành cho đội ngũ biên tập" : "For the editorial team"}</p><Link className="text-link" href="/admin/login">{locale === "vi" ? "Đăng nhập không gian biên tập" : "Sign in to the editorial workspace"}</Link><Link className="text-link" href={`/${locale}/privacy`}>{locale === "vi" ? "Riêng tư và an toàn AI" : "Privacy and AI safety"}</Link><small>{locale === "vi" ? "Tài khoản do quản trị viên cấp; không có đăng ký công khai." : "Accounts are issued by an administrator; public registration is not available."}</small></div>
    </footer>
  </div>;
}
