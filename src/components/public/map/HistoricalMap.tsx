  ...
  return <>
    ...
    <div className="historical-map-layout">
      <figure className="historical-map-figure" aria-labelledby="map-title" data-testid="historical-map">
        ...
        <div className="map-marker-layer" aria-label={vi ? "Điểm địa danh" : "Place markers"}>{places.map((place) => <a key={place.id} href={`#${place.id}`} className={`map-marker motion-context-link ${place.precision === "APPROXIMATE" ? "approximate" : "exact"}`} data-motion-context="map-to-place" style={{ left: `${x(place.point.longitude)}%`, top: `${y(place.point.latitude)}%` }} aria-label={`${place.title} — ${place.precision === "APPROXIMATE" ? approximate : exact}`}><span aria-hidden="true">{place.precision === "APPROXIMATE" ? "~" : "•"}</span></a>)}</div>
