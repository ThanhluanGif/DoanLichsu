...
export async function generateMetadata({params}:{params:Promise<{locale:string}>}):Promise<Metadata>{
  const {locale:raw}=await params;if(raw !== "en" || !isPublicLocale(raw))return{};const locale=raw;const other="vi";const copy=t(locale);
...
export default async function CurriculumCatalogEnglishRoute({params}:{params:Promise<{locale:string}>}){
  const {locale:raw}=await params;if(raw !== "en" || !isPublicLocale(raw))notFound();
