...
export async function generateMetadata({params}:{params:Promise<{locale:string;grade:string}>}):Promise<Metadata>{
  const {locale:raw,grade:rawGrade}=await params;const grade=parseGrade(rawGrade);if(raw !== "en" || !isPublicLocale(raw)||grade===undefined)return{};const locale=raw;const other="vi";
...
export default async function CurriculumGradeEnglishRoute({params}:{params:Promise<{locale:string;grade:string}>}){
  const {locale:raw,grade:rawGrade}=await params;const grade=parseGrade(rawGrade);if(raw !== "en" || !isPublicLocale(raw)||grade===undefined)notFound();
