import { getSources } from "@/lib/content/public-repository";
...
export async function GET(request: Request, context: { params: Promise<{ locale: string }> }) {
  ...
  return withPublicDatabase((database) => getSources(database, parseLocale(rawLocale), new URL(request.url).searchParams));
}
