import { getContents } from "@/lib/content/public-repository";
...
export async function GET(request: Request, context: { params: Promise<{ locale: string }> }) {
  ...
  return withPublicDatabase((database) => getContents(database, parseLocale(rawLocale), new URL(request.url).searchParams));
}
